...
/* $Id: str.h 171 2010-06-14 02:56:03Z wangsl $ */
...
// Str can also be used to convert Fortran characters to C/C++ char pointer
...
class Str
{
  struct StrRep
  {
    ...
    StrRep(const char *s) : ref(1), p(new char[strlen(s)+1])
    { 
      strcpy(p,s);
    }
    
    // For Fotran characters
    StrRep(const char *s, int len_s) : ref(1), p(new char[len_s+1])
    {
      memcpy(p, s, len_s*sizeof(char));
      p[len_s] = '\0';
      assert((int) strlen(p) == len_s);
    }
...
public:
  ...
  Str(const char *s, int ls) : rep(new StrRep(s, ls)) 
  { }
  ...
  Str(double f, const char *format = 0)
  {
    char buf[64];
    if(format)
